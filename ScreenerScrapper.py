...
def getQuarterlyResults(soup):
    ...
    return getGeneralData(section_children)

def getProfitAndLoss(soup):
    section_children = soup.find("section", {"id": "profit-loss"})
    return getGeneralData(section_children)

def getGeneralData(section_children):
    quarters = getHeaderInQuarters(section_children)
...
balancesheet_historical_assets, balancesheet_historical_liability =  getBalanceSheet(soup)

operating_activity_dict, investing_activity_dict, financing_activity_dict, net_cash_flow_dict = getCashFlows(soup)

# print(operating_activity_dict, investing_activity_dict, financing_activity_dict, net_cash_flow_dict)

getQuarterlyResults(soup)
